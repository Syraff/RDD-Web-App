...
export default function InputFile({
  ...
  // Constants
  const MAX_SIZE_MB = 50; // Diperbesar untuk Video

  // Effect: Handle perubahan nilai 'val' dari parent
  useEffect(() => {
    if (!val) {
      setPreviewUrl(null);
      setFileType(null);
    ...
    // Tentukan tipe file berdasarkan properti file
    if (val.type.startsWith("image/")) {
      setFileType("image");
    } else if (val.type.startsWith("video/")) {
      setFileType("video");
    }
  ...
  /**
   * Validasi dan kirim file ke parent state
   */
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_SIZE_MB * 1024 * 1024) {
      ...
      // Reset input value agar user bisa memilih file yang sama lagi jika mau
      if (fileInputRef.current) fileInputRef.current.value = "";
      return;
    }
    ...
    // 3. Set Value ke Parent
    // (Preview akan otomatis terupdate via useEffect saat val berubah)
    setValue(file);
  };
      ...
      >
        ...
        {fileType === "video" && previewUrl && (
          <div className="w-full flex flex-col items-center justify-center">
            <video
              controls
              className="w-full max-h-[300px] rounded-lg shadow-sm bg-black"
              src={previewUrl}
            >
              Your browser does not support the video tag.
            </video>
            <div className="mt-2 text-xs text-gray-700 font-medium bg-gray-100 rounded-full px-3 py-1">
              Video Selected
            </div>
          </div>
        )}
